...
if st.button("🔥 팀 배정 시작"):
    if any(p["닉네임"] == "" for p in player_data):
        ...
        # 팀 간 실력 차이가 3 미만이 될 때까지 반복
        max_attempts = 1000  # 너무 오래 반복되지 않도록 최대 반복 횟수 설정
        attempt = 0
        balanced_teams_found = False
        ...
        while attempt < max_attempts:
            ...
            for pos in positions:
                ...
                random.shuffle(sorted_players)  # 랜덤하게 섞기
            ...
            # ✅ 팀 간 실력 차이가 3 미만이면 출력하고 종료
            if abs(team1_score - team2_score) < 3:
                balanced_teams_found = True
                break

        if balanced_teams_found:
            # ======== 🔥 결과 출력 (카드 스타일) ========
            ...
            st.error("⚠️ 균형 잡힌 팀을 찾지 못했습니다. 다시 시도해 주세요!")
